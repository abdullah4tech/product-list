...
const addCart = (item: { name: string; price: number; count?: number }) => {
  // Check if an item with the same name and price already exists
  const exists = cart.cartCollections.some(
    (cartItem) => cartItem.name === item.name && cartItem.price === item.price
  );

  if (!exists) {
    cart.cartCollections.unshift(item); // Add the item to the beginning of the array
  }
...
const removeItem = (item: { name: string; price: number }) => {
  ...
  if (exists) {
    console.log(cart.cartCollections.indexOf(item));
  } else {
    console.log('Item does not exists for now')
  }
}
  ...
  <section class="p-6 sm:p-12 flex flex-col md:flex-row md:justify-between gap-8">
    ...
    <div class="flex flex-col gap-7 w-full md:w-3/4">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-5">
        ...
        >
          <div class="relative border-2 border-redish rounded-xl">
            <img
              :class="{'blur-sm': !isLoaded}"
              class="rounded-xl w-full object-cover transition duration-500 ease-in-out"
              :src="i.image.mobile"
              @load="onImageLoad"
            ...
            <!-- Add to Cart Button -->
            <div
              :class="{'hidden': cart.cartCollections.length !== 0}"
            ...
            <div
              class="absolute hidden -bottom-4 left-1/2 transform -translate-x-1/2 w-32 flex items-center gap-3 justify-around bg-redish opacity-100 p-2 rounded-full shadow-lg cursor-pointer select-none">
              <div @click="addMoreItem" class="w-4 h-4 flex items-center justify-center rounded-full border border-white">
                <svg xmlns="http://www.w3.org/2000/svg" width="10" height="2" fill="none" viewBox="0 0 10 2">
                  <path fill="#fff" d="M0 .375h10v1.25H0V.375Z" />
                </svg>
              </div>
              <span class="text-white">4</span>
              <div @click="removeItem({name: i.name, price: i.price})" class="w-4 h-4 flex items-center justify-center rounded-full border border-white">
